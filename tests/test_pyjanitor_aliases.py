...
class TestRleId:
    def test_rle_id_groups_runs(self):
        ...
        assert (
            out["_rle_id"].iloc[0] == out["_rle_id"].iloc[2]
...
class TestFilterDate:
    def test_filter_date_range(self):
        dates = pd.DataFrame({"d": pd.to_datetime(["2024-01-01", "2024-06-01", "2024-12-31"])})
...
class TestPivotWiderSpec:
    def test_pivot_wider_spec(self):
        ...
        dj = DuckJanitor.from_pandas(df)
        out = dj.pivot_wider_spec(id_cols=["id"], names_from="year", values_from="value").collect()
