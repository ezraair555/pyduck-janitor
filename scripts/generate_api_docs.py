...
VERIFIED_EXAMPLES: dict[str, str] = {
    ...
    "from_database": "import sqlite3\ndb = sqlite3.connect(':memory:')\ndb.execute(\"CREATE TABLE t (a INTEGER)\")\ndb.execute(\"INSERT INTO t VALUES (1)\")\ndj = DuckJanitor.from_database(db, 'SELECT * FROM t')\ndj.collect()",
    "metric_from_database": "import sqlite3\ndb = sqlite3.connect(':memory:')\ndb.execute(\"CREATE TABLE t (g TEXT, a INTEGER)\")\ndb.executemany(\"INSERT INTO t VALUES (?, ?)\", [('x', 1), ('x', 2)])\nDuckJanitor.metric_from_database(db, 'SELECT g, a FROM t', {'total': ('a', 'sum')}, group_by='g').collect()",
    "asof_join": "left = DuckJanitor.from_pandas(pd.DataFrame({'id': [1], 'event_time': pd.to_datetime(['2024-01-10'])}))\nright = DuckJanitor.from_pandas(pd.DataFrame({'id': [1], 'effective_time': pd.to_datetime(['2024-01-01']), 'value': [10]}))\nleft.asof_join(right, 'event_time', 'effective_time', by='id').collect()",
    "inner_join": "left = DuckJanitor.from_pandas(pd.DataFrame({'id': [1, 2, 3], 'v': [10, 20, 30]}))\nright = DuckJanitor.from_pandas(pd.DataFrame({'id': [1, 2, 99], 'w': [100, 200, 900]}))\nleft.inner_join(right, on='id').collect()",
    "left_join": "left = DuckJanitor.from_pandas(pd.DataFrame({'id': [1, 2, 3], 'v': [10, 20, 30]}))\nright = DuckJanitor.from_pandas(pd.DataFrame({'id': [1, 2, 99], 'w': [100, 200, 900]}))\nleft.left_join(right, on='id').collect()",
    "right_join": "left = DuckJanitor.from_pandas(pd.DataFrame({'id': [1, 2, 3], 'v': [10, 20, 30]}))\nright = DuckJanitor.from_pandas(pd.DataFrame({'id': [1, 2, 99], 'w': [100, 200, 900]}))\nleft.right_join(right, on='id').collect()",
    "full_join": "left = DuckJanitor.from_pandas(pd.DataFrame({'id': [1, 2, 3], 'v': [10, 20, 30]}))\nright = DuckJanitor.from_pandas(pd.DataFrame({'id': [1, 2, 99], 'w': [100, 200, 900]}))\nleft.full_join(right, on='id').collect()",
    "semi_join": "left = DuckJanitor.from_pandas(pd.DataFrame({'id': [1, 2, 3], 'v': [10, 20, 30]}))\nright = DuckJanitor.from_pandas(pd.DataFrame({'id': [1, 2, 99], 'w': [100, 200, 900]}))\nleft.semi_join(right, on='id').collect()",
    "anti_join": "left = DuckJanitor.from_pandas(pd.DataFrame({'id': [1, 2, 3], 'v': [10, 20, 30]}))\nright = DuckJanitor.from_pandas(pd.DataFrame({'id': [1, 2, 99], 'w': [100, 200, 900]}))\nleft.anti_join(right, on='id').collect()",
    "window_mutate": "dj = DuckJanitor.from_pandas(pd.DataFrame({'id': [1, 1], 'value': [10, 20]}))\ndj.window_mutate({'previous': 'LAG(value)'}, partition_by='id', order_by='value').collect()",
    "recursive_cte": "dj = DuckJanitor.from_pandas(pd.DataFrame({'id': [1, 2], 'parent_id': [None, 1]}))\ndj.recursive_cte('tree', 'SELECT id, parent_id, 0 AS depth FROM self WHERE parent_id IS NULL', 'SELECT c.id, c.parent_id, t.depth + 1 FROM self c JOIN tree t ON c.parent_id = t.id').collect()",
    "validate_keys": "dj = DuckJanitor.from_pandas(pd.DataFrame({'id': [1, 2], 'value': [10, 20]}))\ndj.validate_keys('id').collect()",
    "deduplicate": "dj = DuckJanitor.from_pandas(pd.DataFrame({'id': [1, 1], 'as_of': [1, 2]}))\ndj.deduplicate('id', order_by='as_of', keep='last').collect()",
    "filter_noise": "dj = DuckJanitor.from_pandas(pd.DataFrame({'id': [1, 2, 2]}))\ndj.filter_noise(id_col='id', min_records=2).collect()",
    "hierarchy_edges": "dj = DuckJanitor.from_pandas(pd.DataFrame({'employee_id': [2], 'manager_id': [1]}))\ndj.hierarchy_edges('employee_id', 'manager_id').collect()",
    "time_slice": "dj = DuckJanitor.from_pandas(pd.DataFrame({'event_date': pd.to_datetime(['2024-01-01', '2024-02-01'])}))\ndj.time_slice('event_date', start='2024-01-15').collect()",
    "event_window": "dj = DuckJanitor.from_pandas(pd.DataFrame({'event_date': pd.to_datetime(['2024-01-01', '2024-01-10'])}))\ndj.event_window('event_date', '2024-01-10', pre_window='2 days').collect()",
    "change_detection": "dj = DuckJanitor.from_pandas(pd.DataFrame({'id': [1, 1], 'event_date': pd.to_datetime(['2024-01-01', '2024-01-02']), 'status': ['a', 'b']}))\ndj.change_detection('id', 'event_date', columns=['status']).collect()",
    "metrics": "dj = DuckJanitor.from_pandas(pd.DataFrame({'group': ['a', 'a'], 'value': [1, 2]}))\ndj.metrics({'total': ('value', 'sum')}, group_by='group').collect()",
    "profile": "dj = DuckJanitor.from_pandas(pd.DataFrame({'a': [1, 2], 'b': ['x', None]}))\ndj.profile().collect()",
    "metric_cube": "dj = DuckJanitor.from_pandas(pd.DataFrame({'year': [2024, 2024], 'region': ['E', 'W'], 'amount': [1, 2]}))\ndj.metric_cube(['year', 'region'], {'total': 'SUM(amount)'}, totals='rollup').collect()",
    "rate_metrics": "dj = DuckJanitor.from_pandas(pd.DataFrame({'group': ['a', 'a'], 'success': [1, 0]}))\ndj.rate_metrics({'rate': ('SUM(success)', 'COUNT(*)')}, group_by='group').collect()",
    "cohort_metrics": "dj = DuckJanitor.from_pandas(pd.DataFrame({'user_id': [1, 1], 'event_date': pd.to_datetime(['2024-01-01', '2024-02-01'])}))\ndj.cohort_metrics('user_id', 'event_date').collect()",
    "freshness": "dj = DuckJanitor.from_pandas(pd.DataFrame({'event_date': pd.to_datetime(['2024-01-01'])}))\ndj.freshness('event_date').collect()",
    "reconcile": "left = DuckJanitor.from_pandas(pd.DataFrame({'id': [1, 2]}))\nright = DuckJanitor.from_pandas(pd.DataFrame({'id': [2, 3]}))\nleft.reconcile(right, 'id').collect()",
...
def _dedent_doc(doc: str) -> str:
    ...
    out = [lines[0]] + [line[strip:] if len(line) >= strip else "" for line in lines[1:]]
    return "\n".join(out)
...
def render_body_block(title: str, body: str) -> list[str]:
    ...
    for raw_line in body.splitlines():
        ...
        re.match(r"^(\*{0,2})([A-Za-z_][A-Za-z0-9_ \[\],\.]{0,60}?)\1? ?:?", line)
        if (
            line
            ...
            out.append(
                ("    " if line.startswith(" ") else "") + re.sub(r"^-", "•", line)
                if True
                else line
            )
    out.append("")
...
def build_page(check_only: bool = False) -> str | None:
    from pyduck_janitor import DropLabel, DuckJanitor, patterns  # noqa: F401

    getattr(DuckJanitor, "__dict__", {})
    ...
    for name, obj, group in entries:
        ...
        if parsed.parameters:
            ...
            for raw_line in parsed.parameters.splitlines():
                line = raw_line.rstrip()
                if not line.strip():
                    continue
                ...
                name_only = re.match(r"^(\**\w+\**)$", line.strip())
        ...
        if parsed.returns:
            ...
            for line in parsed.returns.splitlines():
                t = line.strip()
                if t:
                    parts.append(t)
            parts.append("")
        ...
        if parsed.raises:
            parts.append("**Raises**\n")
            for line in parsed.raises.splitlines():
                t = line.strip()
                if t:
                    parts.append(t)
            parts.append("")
        ...
        if example:
            ...
            for line in example.splitlines():
                parts.append(">>> " + _prompt_re.sub("", line))
            parts.append("```\n")
        else:
