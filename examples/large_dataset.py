    ...
    chunk = pd.DataFrame(
        {
            ...
            "category": np.random.choice(
                ["Electronics", "Clothing", "Food", "Services"], chunk_size
            ),
...
print("Pipeline built (no data loaded into memory yet)")
...
high_value = cleaned.filter_column("amount", "amount > 500").collect()
...
print("- Out-of-core processing (1M+ rows)")
print("- Lazy evaluation (pipeline built without loading data)")
print("- Query optimization (DuckDB optimizes entire pipeline)")
print("- SQL interoperability (mix janitor methods with SQL)")
