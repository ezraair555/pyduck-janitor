...
cleaned = (
    dj.clean_names()  # Standardize column names
...
print("Pipeline built (lazy evaluation)")
print(f"Query plan:\n{cleaned.explain()}")
...
encoded = DuckJanitor.from_pandas(result).encode_categorical("region").collect()

print("\nWith categorical encoding:")
print(encoded.head())

# One-hot encode
dummies = DuckJanitor.from_pandas(result).get_dummies("region", prefix="reg").collect()
