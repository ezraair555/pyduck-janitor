...
def clean_names(relation: duckdb.DuckDBPyRelation, strip_underscores: bool = True,
    ...
    """
    if case_type not in {"lower", "upper", "original"}:
        raise ValueError("case_type must be one of: 'lower', 'upper', 'original'")
...
def select_columns(relation: duckdb.DuckDBPyRelation, columns: Union[str, List[str]],
    ...
    # pyjanitor ``DropLabel`` parity: DropLabel entries exclude columns.
    from .duck_janitor import DropLabel

    includes = []
    excludes = []

    def _collect(spec):
        """Populate includes/excludes from any column spec entry."""
    ...
    # Apply exclusions from DropLabel entries.
    if excludes:
        if not columns:
            columns = [c for c in relation.columns if c not in excludes]
        else:
